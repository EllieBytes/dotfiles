...
let
  urlTranslated = myLib.translateGitHubURL rootUrl;
    ...
    sudo nixos-rebuild switch --rebuild-lock-file --option eval-cache false --flake $END_PREFIX#$END_SYSNAME
  '';

  end-rebuild-host = pkgs.writeShellScriptBin "end-rebuild-host" ''
    sudo nixos-rebuild switch --rebuild-lock-file --option eval-cache false --flake $END_PREFIX#$1
  '';
  ...
  '';
    ...
    sudo nixos-rebuild switch --rebuild-lock-file --option eval-cache false --flake /etc/nixos/end-cache#$END_SYSNAME
  '';

  end-rebuild-cachedhost = pkgs.writeShellScriptBin "end-rebuild-host-cached" ''
    sudo nixos-rebuild switch --rebuild-lock-file --option eval-cache false --flake /etc/nixos/end-cache#$1
  '';
